...
export default function IGPostComposer({ pages, initialPost }: InstagramPageProps) {
  ...
  const handlePost = async (
    ...
    try {
      ...
      const resPost = await private_api_call({
        path: initialPost
          ? `instagram/edit_post/${initialPost.id}`
...
function ComposerCard({
  ...
  return (
    <section className="bg-card border border-border rounded-2xl p-5 space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h2 className="text-lg font-semibold text-foreground">
            {page?.name || "Your Instagram Page"}
          </h2>

          <p className="text-sm text-muted-foreground">
            {page?.name || "instagram.com/yourpage"}
          </p>
        </div>
      ...
      <InstagramPostPromptDialog open={open} setOpen={setOpen} setMessage={setMessage} />
    </section>
  );
}
  ...
  return (
    <section className="bg-card border border-border rounded-2xl p-5">
      ...
      <button
        ...
        "
      >
        <p className="text-red-500">⊕ Add photos & videos (required)</p>
      </button>
...
export function InstagramPostPromptDialog({ open, setOpen, setMessage }: { open: boolean; setOpen: (v: boolean) => void; setMessage: (v: string) => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Generate Instagram Post</DialogTitle>
        </DialogHeader>
